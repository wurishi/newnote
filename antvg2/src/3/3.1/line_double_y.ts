...
export default function () {
    ...
    function toInteger(number: number, fix = 1) {
    ...
    function humanizeDuration(duration: number, fix = 1) {
        if (duration === 0) {
            return '0';
        }
        if (duration < MINUTE) {
    ...
    chart.line().position('date*time').color('#9AD681').shape('dash');
