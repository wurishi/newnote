...
export default function () {
    ...
    const v1 = chart.createView({ padding: 32 });
    v1.data(data);
    v1.scale('temperature', { alias: '温度区间' });
    v1.area().position('time*temperature');

    const v2 = chart.createView({ padding: 32 });
    v2.data(averages);
    v2.axis(false);
    v2.scale('temperature', { alias: '平均温度' });
