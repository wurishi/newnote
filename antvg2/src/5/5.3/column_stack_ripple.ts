import { Chart, IGroup, Util } from '@antv/g2';
...
export default function () {
    ...
    chart.on('afterrender', () => {
        ...
        let labelGroup = foregroundGroup.findById('customLabels') as IGroup;
        if (labelGroup) {
            labelGroup.clear();
        } else {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (i === count - 1) {
                ...
                const point1 = Util.polarToCartesian(
                    center.x,
                    center.y,
                    pointRadius,
                    (-3 * Math.PI) / 8 + (Math.PI / 8) * i
                );
                let point2: { x: number; y: number };
                if (i === 0) {
                    point2 = { x: bbox.maxX, y: bbox.minY };
                    firstPoint2 = point2;
                    firstPoint1 = point1;
                } else {
                    point2 = {
                        x: Math.min(
                            firstPoint2.x + point1.x - firstPoint1.x,
                            elements[0].getBBox().maxX
                        ),
