...
import DATA from '../../assets/contour/covid-by-county.json'
...
const MyContourLayer = () => {
    const data = DATA
    const week = 35
    const contours = BANDS
    const cellSize = 60000
    const mapStyle =
    ...
    const layers = [
        new ContourLayer({
            ...
            getWeight: (d: any) =>
                ((d.casesByWeek[week] || 0) / d.population) * 1e5,
            updateTriggers: { getWeight: week },
            pickable: true,
            aggregation: 'MAX',
            contours,
            cellSize,
