...
export class LabelLineSystem extends BasicSubSeriesSystem {
  constructor() {
    super(['line', 'bar', 'scatter', 'effectScatter'], 'labelLine', '', true);
  }
  buildSeriesUI(p: iSubBuildSeriesUIParams) {
    ...
    ui.add(obj, 'length2', -100, 100, 1).onChange(changeOptions);

    obj.smooth = obj.smooth || 0;
    ui.add(obj, 'smooth', 0, 1, 0.1).onChange(changeOptions);

    obj.minTurnAngle = obj.minTurnAngle || 0;
