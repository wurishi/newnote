...
export default class implements iSub {
  ...
  main(): HTMLCanvasElement {
    gui = new GUI();
    gui.add(api, 'u_shadow', { physically: 0, cheap: 1 });
    gui.add(api, 'u_sphere');
    return createCanvas();
  }
  userFragment(): string {
    return fragment;
  ...
  initial?(gl: WebGLRenderingContext, program: WebGLProgram): Function {
    ...
    const u_sphere = webglUtils.getUniformLocation(gl, program, 'u_sphere');
    return () => {
      u_shadow.uniform1i(api.u_shadow);
      u_sphere.uniform1i(api.u_sphere ? 1 : 0);
    };
  }
}
