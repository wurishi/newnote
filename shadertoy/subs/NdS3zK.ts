...
#ifdef CUBEMAP
    return inv_gamma(texture(iChannel2, rayDir).rgb);
#else 
...
float getSDF(vec3 p, float sdfSign){
    ...
    float strength = mix(0.02, 0.1, smoothstep(-0.6, -1.5, p.y+0.4));
...
vec3 getEnvironment(vec3 org, vec3 rayDir, out vec3 transmittace){
        float sdfSign = -1.0;
        ...
        transmittace = exp( -d * (1.0 - waterColour));
        
        vec3 result = transmitted * transmittace;
        return result;
}
...
vec3 shadingPBR(vec3 cameraPos, vec3 lightPos, vec3 p, vec3 n, vec3 rayDir, vec3 geoNormal){
    ...
    result += (1.0-f) * CLARITY 
            * getEnvironment(p+rayDir*2.0*EPSILON, refract(rayDir, n, ETA), transmittace);
    
   
    float mu = dot(refract(rayDir, n, ETA), lightDir);
    float phase = mix(HenyeyGreenstein(-0.3, mu), HenyeyGreenstein(0.85, mu), 0.5);
    
    result += sunLightColour * transmittace * phase;
