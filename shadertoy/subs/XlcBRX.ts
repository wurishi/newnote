...
export default class implements iSub {
  ...
  name(): string {
    return 'Triangulated Heightfield Trick';
  }
  sort() {
    return 173;
  }
  ...
  initial?(gl: WebGLRenderingContext, program: WebGLProgram): Function {
    const u_single_sample = webglUtils.getUniformLocation(
      gl,
      program,
      'u_single_sample'
    );
    return () => {
      u_single_sample.uniform1i(api.u_single_sample ? 1 : 0);
    };
  }
}
