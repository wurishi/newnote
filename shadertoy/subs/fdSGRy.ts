...
Material shadeModel(Model model, inout vec3 nor) {
    ...
    // floorboards
    if (id == 2) {
        vec3 col = vec3(0.714,0.43,0.19);
        col = mix(col * vec3(.5,.3,.2), col + .1, .9);
        vec2 size = vec2(.1,.02);        
...
export default class implements iSub {
  ...
  initial?(gl: WebGLRenderingContext, program: WebGLProgram): Function {
    const u_sub = webglUtils.getUniformLocation(gl, program, 'u_sub');
    return () => {
      u_sub.uniform1i(api.u_sub ? 1 : 0);
    };
  }
  // channels() {
