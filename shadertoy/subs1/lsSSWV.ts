...
export default class implements iSub {
  ...
  main(): HTMLCanvasElement {
    gui = new GUI();
    gui.add(api, 'u_no_antialias');
    gui.add(api, 'u_sort').name('sort(仅no_antialias==false)');
    return createCanvas();
  }
  userFragment(): string {
    return fragment;
  ...
  initial?(gl: WebGLRenderingContext, program: WebGLProgram): Function {
    const u_no_antialias = webglUtils.getUniformLocation(
      gl,
      program,
      'u_no_antialias'
    );
    const u_sort = webglUtils.getUniformLocation(gl, program, 'u_sort');
    return () => {
      u_no_antialias.uniform1i(api.u_no_antialias ? 1 : 0);
      u_sort.uniform1i(api.u_sort ? 1 : 0);
    };
  }
  channels() {
    return [
