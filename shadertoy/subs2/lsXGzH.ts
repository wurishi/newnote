...
const fragment = `
uniform int u_raymarchMax;
uniform bool u_enable_ambient;
uniform bool u_transparency;
uniform bool u_double_sided_transparency;
uniform bool u_enable_specular;
uniform bool u_enable_reflections;
uniform bool u_enable_shadows;
uniform bool u_enable_fog;
uniform bool u_enable_directional_light;
uniform bool u_enable_directional_light_flare;
uniform bool u_enable_point_light;
uniform bool u_enable_point_light_flare;

uniform float kPipeRadius;
uniform float kPipeThickness;
...
void ApplyAtmosphere(inout vec3 col, const in C_Ray ray, const in C_HitInfo hitInfo)
{
  if(u_enable_fog) {
    ...
    if(u_enable_directional_light_flare) {
...
vec3 GetTransmission( const in C_Ray ray, const in C_HitInfo hitInfo, const in C_Surface surface, const in C_Material material )
{
    if(u_transparency)
    {
        ...
        if(u_double_sided_transparency) {
          C_HitInfo hitInfo2;
          ...
          fExtinctionDist = hitInfo2.fDistance;
          vSceneColour = GetSceneColourSecondary(refractRay2);
        }
        else {
          vSceneColour = GetSceneColourSecondary(refractRay);                                                                        
			    fExtinctionDist = 0.5;
        }
...
const api = {
  u_raymarchMax: 32,
  u_enable_ambient: true,
  u_transparency: true,
  u_double_sided_transparency: true,
  u_enable_specular: true,
  u_enable_reflections: true,
  u_enable_shadows: true,
  u_enable_fog: true,
  u_enable_directional_light: true,
  u_enable_directional_light_flare: true,
  u_enable_point_light: false,
...
export default class implements iSub {
  ...
  main(): HTMLCanvasElement {
    gui = new GUI();
    gui.add(api, 'u_raymarchMax', 2, 256);
    gui.add(api, 'u_enable_ambient');
    gui.add(api, 'u_transparency');
    gui.add(api, 'u_double_sided_transparency');
    gui.add(api, 'u_enable_specular');
    gui.add(api, 'u_enable_reflections');
    gui.add(api, 'u_enable_shadows');
    gui.add(api, 'u_enable_fog');
    gui.add(api, 'u_enable_directional_light');
    gui.add(api, 'u_enable_directional_light_flare');
    gui.add(api, 'u_enable_point_light');
    gui.add(api, 'u_enable_point_light_flare');
  ...
  initial?(gl: WebGLRenderingContext, program: WebGLProgram): Function {
    const u_raymarchMax = webglUtils.getUniformLocation(
      gl,
      program,
      'u_raymarchMax'
    );
    const u_enable_ambient = webglUtils.getUniformLocation(
      gl,
      program,
      'u_enable_ambient'
    );
    const u_transparency = webglUtils.getUniformLocation(
      gl,
      program,
      'u_transparency'
    );
    const u_double_sided_transparency = webglUtils.getUniformLocation(
      gl,
      program,
      'u_double_sided_transparency'
    );
    const u_enable_specular = webglUtils.getUniformLocation(
      gl,
      program,
      'u_enable_specular'
    );
    const u_enable_reflections = webglUtils.getUniformLocation(
      gl,
      program,
      'u_enable_reflections'
    );
    const u_enable_shadows = webglUtils.getUniformLocation(
      gl,
      program,
      'u_enable_shadows'
    );
    const u_enable_fog = webglUtils.getUniformLocation(
      gl,
      program,
      'u_enable_fog'
    );
    const u_enable_directional_light = webglUtils.getUniformLocation(
      gl,
      program,
      'u_enable_directional_light'
    );
    const u_enable_directional_light_flare = webglUtils.getUniformLocation(
      gl,
      program,
      'u_enable_directional_light_flare'
    );
    const u_enable_point_light = webglUtils.getUniformLocation(
      gl,
      program,
      'u_enable_point_light'
    );
    const u_enable_point_light_flare = webglUtils.getUniformLocation(
      gl,
      program,
      'u_enable_point_light_flare'
    );

    const kPipeRadius = webglUtils.getUniformLocation(
      gl,
      program,
      'kPipeRadius'
    );
    const kPipeThickness = webglUtils.getUniformLocation(
      gl,
      program,
      'kPipeThickness'
    );
    const kPipeHeight = webglUtils.getUniformLocation(
      gl,
      program,
      'kPipeHeight'
    );
    ...
    return () => {
      ...
      u_transparency.uniform1i(api.u_transparency ? 1 : 0);
      u_double_sided_transparency.uniform1i(
        api.u_double_sided_transparency ? 1 : 0
      );
      u_enable_specular.uniform1i(api.u_enable_specular ? 1 : 0);
      u_enable_reflections.uniform1i(api.u_enable_reflections ? 1 : 0);
      u_enable_shadows.uniform1i(api.u_enable_shadows ? 1 : 0);
      u_enable_fog.uniform1i(api.u_enable_fog ? 1 : 0);
      u_enable_directional_light.uniform1i(
        api.u_enable_directional_light ? 1 : 0
      );
      u_enable_directional_light_flare.uniform1i(
        api.u_enable_directional_light_flare ? 1 : 0
      );
      u_enable_point_light.uniform1i(api.u_enable_point_light ? 1 : 0);
      u_enable_point_light_flare.uniform1i(
        api.u_enable_point_light_flare ? 1 : 0
      );
  ...
  channels() {
    return [
      { type: 0, path: './textures/lsXGzH.jpg', ...webglUtils.TEXTURE_MIPMAPS },
    ];
  }
}
