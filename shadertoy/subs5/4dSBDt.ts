...
float noise( in vec3 x )
{
    ...
    return textureLod(iChannel2, (p+f+0.5).xy/32.0, 0.0).x;
#else
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for(float i = 0.; i < NUM_SAMPLES; i++)
    {
        ...
        sum += textureLod(iChannel2, uv, 0.0);
    }
    const float BLOOM_AMOUNT = 0.05;
    sum.xyz = mix(textureLod(iChannel2, q, 0.0).xyz, sum.xyz / NUM_SAMPLES, BLOOM_AMOUNT);
