...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 uv =    texture(iChannel1, fract(vUv)).xyz;
    vec3 uv_n =  texture(iChannel1, fract(vUv+n)).xyz;
    vec3 uv_e =  texture(iChannel1, fract(vUv+e)).xyz;
    vec3 uv_s =  texture(iChannel1, fract(vUv+s)).xyz;
    vec3 uv_w =  texture(iChannel1, fract(vUv+w)).xyz;
    vec3 uv_nw = texture(iChannel1, fract(vUv+nw)).xyz;
    vec3 uv_sw = texture(iChannel1, fract(vUv+sw)).xyz;
    vec3 uv_ne = texture(iChannel1, fract(vUv+ne)).xyz;
    vec3 uv_se = texture(iChannel1, fract(vUv+se)).xyz;
    ...
    vec3 init = texture(iChannel0, fragCoord.xy / iResolution.xy).xyz;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec4 init = texture(iChannel3, fragCoord.xy / iResolution.xy);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    fragColor = contrast(texture(iChannel2, uv), SIGMOID_CONTRAST);
}
`;
...
export default class implements iSub {
  ...
  name(): string {
    return 'Anisotropic Blur Image Warp';
  }
  // sort() {
  //   return 0;
  // }
