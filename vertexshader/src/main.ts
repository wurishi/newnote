...
class VS {
  ...
  constructor() {
    ...
    if (gl) {
      ...
      console.log(
        'can ' + (s.canUseFloat ? '' : 'not ') + 'use floating point textures'
      );
      console.log(
        'can ' +
      ...
      s.sets = {
        ...
        audio: {
          ...
          mode: 'LINES',
          sound:
            'https://soundcloud.com/caseandpoint/case-point-upgrade-free-download',
          lineSize: 'NATIVE',
          backgroundColor: [0, 0, 0, 1],
      ...
      s.streamSource.on('newSource', (source: MediaStreamAudioSourceNode) => {
  ...
  public queueRender(force?: boolean) {
    if (
      (!this.g.requestId &&
        (force || !this.g.wasRendered || (s.running && !this.g.pause))) ||
      this.g.animRects.length
    ) {
      this.g.requestId = requestAnimationFrame((time) => this.render(time));
    }
  }
  ...
  public renderScene(
    ...
    const size = settings.lineSize === 'NATIVE' ? 1 : Number(settings.lineSize);
    gl.lineWidth(size);
    ...
    if (programInfo) {
      ...
      this.uniforms.soundRes = [s.numSoundSamples, s.numHistorySamples] as any;
...
function init() {
  if (!vs) {
    ...
    f.add(ui, 'url', ['audio1.mp3', '128.mp3'])
      .name('Audio')
      .onChange((v) => {
        if (!playFlag) {
          playFlag = true;
